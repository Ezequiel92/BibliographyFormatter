########################################################################################
# Julia script for joining .bib/.bibtex files into a single file with a desire format. 
########################################################################################
...
# Arguments 
- `fields::Array{String,1}`: Ordered list of bib fields to be included in each entry. 
  If one field does not exist it will be ignored. The order of the fields will be 
  respected in the final output.
...
function bib_formatter(fields::Array{String,1}, source_path::String)::String
	...
	# Now `new_bib[key].fields` contains all the data edited above as strings.
	...
	for key in keys(new_bib)
		...
		for field in fields
			if field in available
				...
				if field == "title"
					new_bib[key].fields[field] = strip(new_bib[key].fields[field], ['{', '}', ' '])
				end
