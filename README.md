...
### ‼️ You should always check the end result for mistakes or missing data. The goal of the script is to format the entries, so incorrect or invalid data may produce no warning.

## ℹ️ Some things to note

- This is just a module. It defines two functions (only one of which is exported), and no data structures or global variables.
